...
def PEPR_3611(S_ACC, CY_CC, SECON_ACC, EXCLU_CYCC, EXCLU_CC): #EXCLU_CYCC:CY-399|IC-999, EXCLU_CC:3744
    ...
    global result1
    result1 = pd.concat([df_wocy,df2_cy])
    print(result1)
    result1 = pd.DataFrame(result1)#######################이게 cctr중복제거하려고 넣은값 만약에 오류해결안되면 지우자#############
    
    result1 = result1.drop_duplicates(['Costcenter'], keep = 'first', ignore_index = True) #######################이게 cctr중복제거하려고 넣은값 만약에 오류해결안되면 지우자############################################
...
df_cy399sd = pd.DataFrame({'CY-399SD'}) # 그냥 값체크없이 CY-399SD를 하나의 DF로 만들어서 추가함 # 이 라인은 S90820에만 필요함
result1 = pd.concat([result1 ,df_cy399sd])
